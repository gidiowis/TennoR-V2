...
zokou({nomCom : "instagram" , categorie : "Download"},async (dest , zk , commandeOptions)=>{
  ...
  try {
    ...
    zk.sendMessage(dest,{video : {url : igvid.data.data.data[0].url},caption : "ig video downloader powered by *Tennor-Md*",gifPlayback : false },{quoted : ms}) 
    }
    else {
  ...
  try {
    ...
    .then((result) => {
      ...
      `;
       ...
       zk.sendMessage(dest, { video: { url: result.hd  }, caption: 'facebook video downloader powered by *Tennor-MD*' }, { quoted: ms });
